...
public:
  ...
  DLinkedList* insert(T x){
    Node<T>* temp = header->getNext();
    while(temp != tailer && t->getKey()<x) temp = temp->getNext();
...
template <class T> class CList : public DLinkedList<T>{
private:
  Node<T>* token;
...
public:
  ...
  T* getToken(){
    if(token != this->header) return &(token->getKey());
    return NULL;
  }
  void move(){
    if(token == this->tailer) token = this->header->getNext();
    token = token->getNext();
  }
...
int main(){
  DLinkedList* l = new DLinkedList<int>();
